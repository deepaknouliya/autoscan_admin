...
const CreateBrand = (props) => {
  ...
  const createNewBrand = (payload) => {
    console.log("payload");
    console.log(payload);
    axios
      .post(`${process.env.REACT_APP_API_URL}/brand/create-brand`, payload, {
        headers: {
      ...
      .catch(function (error) {
        toast.error(
          error?.response?.data?.msg ||
            "Brand already exists. Unable to create a new Brand.",
          {
            position: toast.POSITION.TOP_RIGHT,
          }
        );
            ...
            <div className="breadcums-list">
              <ul className="margin-remove m-0">
                <li className="margin-remove padding-remove">
                  <Link to="/create-brand" className="active-btn">
                    Manage Brand
                  </Link>
                </li>
              </ul>
